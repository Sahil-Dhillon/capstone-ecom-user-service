...
public class CartService {
     ...
     */
    public CartItems addItemToCart(int productId, int quantity, String variations, String userId) {
...
//            cartRepo.save(cart);
        ...
        if (matchingCartItem != null) {
            ...
            Products product = productService.listById(productId);
            matchingCartItem.setProductId(productId);
            matchingCartItem.setBrand(product.getBrand());
            matchingCartItem.setDescription(product.getDescription());
            matchingCartItem.setImage(product.getProfileImgUrl());
            matchingCartItem.setName(product.getName());
            matchingCartItem.setPrice(product.getPrice());
...
//        return mapToCartItemDTO(matchingCartItem);
    ...
    public String deleteCartItem(int id,int cartId) {
        // Fetch the cart by cartId
//    	System.out.println(cartRepo.findById(cartId).get());
...
//                .orElseThrow(() -> new IllegalArgumentException("CartItem not found in Cart with ID: " + cartItem.getCartItemId()));
        CartItems itemToRemove = cartItemsRepo.findById(id).get();
    ...
    public String emptyCart(int cartId) {
        Cart cart = cartRepo.findById(cartId).get();
                if(cart==null) {throw new IllegalArgumentException("Cart not found with ID: " + cartId);}
        List<Integer> ids = cart.getListOfCartItems().stream().map((x)->x.getCartItemId()).collect(Collectors.toList());
        // Remove the cartItem from the list
...
//        cartRepo.saveAndFlush(cart);
        ...
        // Delete the CartItem from the CartItems table
        cartItemsRepo.deleteAllByIdInBatch(ids);
 
        return "Cart successfully dumped";
    }
